...
```bash
TranslationProcessor$ sam build
```
...
```bash
TranslationProcessor$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
TranslationProcessor$ sam local start-api
TranslationProcessor$ curl http://localhost:3000/
```
...
```bash
TranslationProcessor$ sam logs -n HelloWorldFunction --stack-name TranslationProcessor --tail
```
...
```bash
TranslationProcessor$ dotnet test test/HelloWorld.Test
```
...
```bash
sam delete --stack-name TranslationProcessor
```
